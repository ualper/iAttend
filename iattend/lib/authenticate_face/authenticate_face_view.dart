...
import 'package:iattend/authenticate_face/scanning_animation/animated_view.dart';
import 'package:iattend/authenticate_face/user_details_view.dart';
...
import 'package:iattend/common/utils/extract_face_feature.dart';
import 'package:iattend/common/views/camera_view.dart';
...
// A function to calculate the Euclidean distance between two points
  ...
  _matchFaces() async {
    ...
    for (List user in users) {
      ...
      setState(() {
        _similarity = split!.matchedFaces.isNotEmpty ? (split.matchedFaces[0]!.similarity! * 100).toStringAsFixed(2) : "error";
  ...
  _fetchUserByName(String orgID) {
    FirebaseFirestore.instance.collection("users").where("organizationId", isEqualTo: orgID).get().catchError((e) {
      log("Getting User Error: $e");
      setState(() => isMatching = false);
      _playFailedAudio;
      CustomSnackBar.errorSnackBar("Something went wrong. Please try again.");
